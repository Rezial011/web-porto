...
import miyam from "../../assets/porto/miyam.png";
...
const porto = [
  ...
  },
  {
    id: 2,
    title: "Miyam F&B",
    desc: "Food ordering website, allow users to browse menu, place orders, and manage their account, while admin can manage menu, status orders, and other data through admin dashboard.",
    imgSrc: miyam,
    bgColor: "#3d3d3d",
    color: "#eeeeee",
    position: "left",
    link: "https://miyam-noodle.vercel.app/",
  },
  {
    id: 3,
    title: "TV-Shows",
...
export default function Carousel() {
  ...
  const goToPrevious = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? porto.length - 1 : prevIndex - 1,
    );
  };

  const goToNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === porto.length - 1 ? 0 : prevIndex + 1,
    );
  };
      ...
      <div className="carousel overflow-hidden">
        ...
        >
          {porto.map((index) => (
            <div
              key={index.id}
              className="carousel-items w-full flex-shrink-0 flex items-center p-4"
              style={{ backgroundColor: `${index.bgColor}` }}
            >
              <div
                alt={`Slide ${index.id + 1}`}
                className="porto-img min-w-[350px] h-full"
              >
                <img
                  src={index.imgSrc}
              ...
              <div className="px-[45px]">
                <h1
                  className="text-4xl title mb-5 "
                  style={{ color: `${index.color}` }}
                >
                  {index.title}
                </h1>
                <p className="font-medium" style={{ color: `${index.color}` }}>
                  {index.desc}
                </p>
                <br />
                <br />
                <a
                  href={index.link}
                  target="_blank"
                  className="prim-red font-bold hover:underline"
                >
                  Project Preview 🚀
                </a>
              </div>
